...
//controllo se amministratore e richiamo la funzione con la lista dei corsi
if (isset($_SESSION["amministratore"]) && $_SESSION["amministratore"] == 1) {
...
// se il bottone submit cancella è stato cliccato (true)
if (isset($_POST['cancella'])) {
    $id_corso = []; //array per memorizzare gli id corso da cancellare
    $conn = connessione_database(); 
    $id_corsi = isset($_POST['id_corso']) ? $_POST['id_corso'] : array(); //ottiene gli id 
    foreach ($id_corsi as $id_corso) {
        $risultato = cancellaCorso($id_corso); //richiama la funzione cancellaCorso
        if ($risultato) { //verifica della cancellazione 
